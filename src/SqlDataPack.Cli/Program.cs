...
static async Task<int> RunExportAsync(ExportCommand command, ParseResult parseResult, CancellationToken cancellationToken) {
    var request = command.Bind(parseResult);
    ...
    var result = await SqlData.ExportAsync(request.ConnectionString, request.OutputPath, request.Options, cancellationToken);
...
static async Task<int> RunImportAsync(ImportCommand command, ParseResult parseResult, CancellationToken cancellationToken) {
    ...
    var result = await SqlData.ImportAsync(request.PackagePath, request.ConnectionString, request.Options, cancellationToken);
...
static bool IsVerbose(ParseResult parseResult, ExportCommand export, ImportCommand import) => parseResult.GetResult(export.Verbose) is OptionResult { Implicit: false } || parseResult.GetResult(import.Verbose) is OptionResult { Implicit: false };
